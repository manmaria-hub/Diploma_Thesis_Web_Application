...
// CSS Styles
import '../../../../src/styles/pages/FOR_ALL_PAGES/COURSES_INFO/POSTGRADUATE_COURSES_LIST/postCoursesContainer.scss'

const PostGraduateCoursesListContainer = () => {
    ...
    // Setting Variables for the ποστgraduate courses group by specialization field
    let [coursesBySpecializationField, setCoursesBySpecializationField] = useState([]);
    ...
    if (coursesByStudyProgram?.length === 0) {
        CoursesResolvers.group_post_courses_by_study_program()
            .then(result => {
                setCoursesByStudyProgram(result?.data?.groupPostCoursesByStudyProgram);
            })
            .catch(err => {
                console.log(err);
                throw err;
    ...
    if (coursesBySemester?.length === 0) {
        CoursesResolvers.group_post_courses_by_semester()
            .then(result => {
                setCoursesBySemester(result?.data?.groupPostCoursesBySemester);
            })
            .catch(err => {
                console.log(err);
                throw err;
    ...
    if (coursesByCourseType?.length === 0) {
        CoursesResolvers.group_post_courses_by_course_type()
            .then(result => {
                setCoursesByCourseType(result?.data?.groupPostCourseByCourseType);
            })
            .catch(err => {
                console.log(err);
                throw err;
    ...
    // Get the undergraduate courses group by specialization field
    if (coursesBySpecializationField?.length === 0) {
        CoursesResolvers.group_post_courses_by_specialization_field()
            .then(result => {
                setCoursesBySpecializationField(result?.data?.groupPostCourseBySpecializationField);
            })
            .catch(err => {
                console.log(err);
                throw err;
    ...
    const formatStudyProgramName = (studyProgram) => {
        ...
        else if (studyProgram === 'ΟΜΑΔΑ ΤΕΧΝΟΛΟΓΙΩΝ ΥΛΙΚΟΥ ΚΑΙ ΑΡΧΙΤΕΚΤΟΝΙΚΗΣ ΤΩΝ ΥΠΟΛΟΓΙΣΤΩΝ') {
            name = 'Ομάδα Υλικού και Αρχιτεκτονικής Υπολογιστών (ΥΑ)';
        }
        else if (studyProgram === 'ΟΜΑΔΑ ΕΦΑΡΜΟΓΩΝ ΚΑΙ ΘΕΜΕΛΙΩΣΕΩΝ ΤΗΣ ΕΠΙΣΤΗΜΗΣ ΤΩΝ ΥΠΟΛΟΓΙΣΤΩΝ') {
            name = 'Ομάδα Εφαρμογών και Θεμελειώσεων της Επιστήμης των Υπολογιστών (ΕΘ)';
        }
        else if (studyProgram === 'ΟΜΑΔΑ ΤΕΧΝΟΛΟΓΙΩΝ ΛΟΓΙΣΜΙΚΟΥ ΚΑΙ ΠΛΗΡΟΦΟΡΙΑΚΩΝ ΣΥΣΤΗΜΑΤΩΝ') {
            name = 'Ομάδα Τεχνολογιών Λογισμικού και Πληροφοριακών Συστημάτων (ΛΠ)';
        }
        else if (studyProgram === 'ΟΜΑΔΑ ΣΗΜΑΤΩΝ, ΤΗΛΕΠΙΚΟΙΝΩΝΙΩΝ ΚΑΙ ΔΙΚΤΥΩΝ') {
            name = 'Ομάδα Σημάτων, Τηλεπικοινωνιών και Δικτύων (ΣΤ)';
        }
    ...
    const findCourseBasicInfo = (courseCode, groupCategory) => {
        ...
        else if (groupCategory === 'specialization_field') {
            coursesBySpecializationField.forEach((studyProgram) => {
                studyProgram.course.forEach((course) => {
                    if (course?.StudyProgram?.course_code === courseCode) {
                        returnedObject = {courseCode : course?.StudyProgram?.course_code, courseName : course?.StudyProgram?.course_name}
                    }
            ...
            <div className="courses_list_main">
                <div className="scroll">
                    <div className="header">
                        ...
                        <div className="title"> Μαθήματα ΠΜΣ
                            <p>Κατάλογος Μαθημάτων Προγράμματος Μεταπτυχιακών Σπουδών</p>
                        </div>
                    ...
                    <div className="forms_container"> 
                        <div className="text">Τα προσφερόμενα <strong style={{fontFamily:'-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji"', display:'inline-flex', marginLeft:'1px', marginRight:'4px', color:'#17a2b8'}}>μαθήματα του Προγράμματος Μεταπτυχιακών Σπουδών</strong>, ανά πρόγραμμα μεταπτυχιακών σπουδών, ανά γνωστικό τομέα, εξάμηνο και τύπο δίδονται παρακάτω. Επιπλέον για κάθε μάθημα σημειώνεται οι ώρες διδασκαλίας, οι μονάδες ECTS τους και οι Διδακτικές τους Μονάδες και γνωστοποιούνται προς ενημέρωσής σας ο Υπεύθυνος Καθηγητής του μαθήματος καθώς και ο/οι διδάσκων/διδάσκοντες αυτού. Διευκρινίζεται ότι με τα εικονίδια: </div>   
                    ...
                    <div className='root'>
                        <h1 className="main_title">Μαθήματα Μεταπτυχιακών Σπουδών</h1>
                        <div className="navigateToSearch">
                            ...
                            <Tabs
                                ...
                                justify
                            >
                                <Tab eventKey="firsthome" title="Ανά Πρόγραμμα Σπουδών">
                                    {coursesBySpecializationField?.map((studyProgram, index)=> {
                                        return(
                                        <TreeView
                                            key={index}
                                            aria-label="gmail"
                                                    ...
                                                    bgColorForDarkMode="#071318">
                                                        <Table striped bordered hover
                                                            ...
                                                            <tbody>
                                                                ...
                                                                <td style={{textAlign:'left', fontWeight:'500', color:'#606060', fontFamily:'-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji"'}}>{courseIn?.StudyProgram.semester}</td> 
                                                                </tr>
                                                                <tr style={{backgroundColor:'white'}}>
                                                                        ...
                                                                        return (
                                                                            <ListGroup> 
                                                                                <ListGroup.Item style={{display:'grid', padding:'2px 2px', backgroundColor:'transparent', verticalAlign:'middle', border:'0px solid gray', gridTemplateColumns:'0fr 0fr 2fr', columnGap:'6px',alignItems:'center'}}>
                                                                                    <BsFillArrowRightCircleFill style={{fontSize:'12px',marginBottom:'-0.15rem', verticalAlign:'middle', color:'red'}}/> <h6 className='course_code' onClick={()=>navigate('/uth-ece/studies/undergraduate/courses/'+ findCourseBasicInfo(preCourse, 'specialization_field').courseCode)}>{findCourseBasicInfo(preCourse, 'specialization_field').courseCode}</h6><p className="course_name">{findCourseBasicInfo(preCourse, 'specialization_field').courseName}</p>                                                                   
                                                                                </ListGroup.Item>   
                                                                            </ListGroup>                                                                            
                                                                        )
                                                                    })} 
                                                                ...
                                                                <td style={{textAlign:'left', fontWeight:'500', color:'#606060', fontFamily:'-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji"'}}>
                                                                    {courseIn?.StudyProgram.prerequisites.map((preCourse) => {
                                                                        return (
                                                                            <ListGroup> 
                                                                                <ListGroup.Item style={{display:'grid', padding:'2px 2px', backgroundColor:'transparent', verticalAlign:'middle', border:'0px solid gray', gridTemplateColumns:'0fr 0fr 2fr', columnGap:'6px',alignItems:'center'}}>
                                                                                    <BsFillArrowRightCircleFill style={{fontSize:'12px',marginBottom:'-0.15rem', verticalAlign:'middle', color:'red'}}/> <h6 className='course_code'>{findCourseBasicInfo(preCourse , 'specialization_field').courseCode}</h6><p className="specialization_field">{findCourseBasicInfo(preCourse, 'specialization_field').courseName}</p>                                                                   
                                                                                </ListGroup.Item>   
                                                                            </ListGroup>                                                                            
                                                                        )
                                                                    })} 
...
export default PostGraduateCoursesListContainer;